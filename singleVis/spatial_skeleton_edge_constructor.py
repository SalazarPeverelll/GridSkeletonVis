...
class SpatialEdgeConstructor(SpatialEdgeConstructorAbstractClass):
    ...
    def _find_nearest_skeleton(self, train_data, skeleton_data):
      
        nearest_neighbor = NearestNeighbors(n_neighbors=1).fit(skeleton_data)
...
class SingleEpochSpatialEdgeConstructor(SpatialEdgeConstructor):
    ...
    def construct(self):
        ...
        #### find nearest skeleton for each training data
        nearest_skeleton_distances , nearest_skeleton_indices = self._find_nearest_skeleton(train_data, self.skeleton_sample)
        ...
        # sk_complex,_,_,_ = self._construct_boundary_wise_complex_skeleton(train_data, self.space_border)
        edge_to, edge_from, weight = self._construct_active_learning_step_edge_dataset(complex, bw_complex,ske_complex)
        
        #### add nearest skeleton to edge
        train_data_indices = np.arange(len(train_data))
        added_edge_from = train_data_indices

        # adjust the index
        added_edge_to = nearest_skeleton_indices.squeeze() + len(train_data) + len(border_centers) - len(self.skeleton_sample)
      
        # use inverse as weight
        added_weight = 1.0 / (nearest_skeleton_distances.squeeze() + 1e-5) 
        # add new edge
